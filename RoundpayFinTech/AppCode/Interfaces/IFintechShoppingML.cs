...
{
    interface IFintechShoppingML
    {
        #region ShoppingSetting
        bool IsShoppingDomain();
        ResponseStatus RedirectLoginCheck(int WId, string SessionId, int UserId);
        Task<IResponseStatus> DoLogout(int ULT, int UserID, int SType);
        UserBalnace GetUserBalnace(int UID = 0);
        ...
        IEnumerable<FilterWithOptions> GetRequeiredFilter(int s2id, int sid = 0);
        ...
        ResponseStatus AddToWishList(int ProductDetailID);
        ResponseStatus RemoveFromWishList(int ProductDetailID, int id = 0, bool RemoveAll = false);
