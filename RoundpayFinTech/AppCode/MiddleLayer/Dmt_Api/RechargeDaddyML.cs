...
{
    public class RechargeDaddyML : IMoneyTransferAPIML
    {
        ...
        public RechargeDaddyML(IHttpContextAccessor accessor, IHostingEnvironment env, IDAL dal)
        {
            _accessor = accessor;
            _env = env;
        ...
        private RechargeDaddySetting AppSetting()
        {
            ...
            try
            {
                ...
                {
                    MobileNo = Configuration["DMR:RDADDY:MobileNo"],
                    APIKey = Configuration["DMR:RDADDY:APIKey"],
                    AgentCode = Configuration["DMR:RDADDY:AgentCode"],
                    Version = Configuration["DMR:RDADDY:Version"],
                    BASEURL = Configuration["DMR:RDADDY:BASEURL"]
                };
            }
            catch (Exception ex)
            {
    ...
    <soap:Body>
        ...
        public DMRTransactionResponse AccountTransfer(MTAPIRequest request)
        {
            ...
            var objReq = new SendMoneyNTDReq
            {
                ...
                CUR = "INR",
                AG_LAT = request.Lattitude,
                AG_LONG = request.Longitude
            };
                ...
                if (!string.IsNullOrEmpty(responseJson))
                {
                    ...
                    if (_apiRes != null)
                    {
                        ...
                        if ((_apiRes.NTDRESP.STATUSCODE ?? -100) == 0)
                        {
                            ...
                            {
                                res.LiveID = _apiRes.NTDRESP.BANKREFNO;
