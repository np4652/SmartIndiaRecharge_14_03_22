...
{
    public class PayUDmtML : IMoneyTransferAPIML
    {
        ...
        public PayUDmtML(IHttpContextAccessor accessor, IHostingEnvironment env, IDAL dal)
        {
            _accessor = accessor;
            _env = env;
        ...
        private PayUSetting AppSetting()
        {
            ...
            try
            {
                ...
                {
                    CLIENTID = Configuration["DMR:PAYUDMT:CLIENTID"],
                    USERNAME = Configuration["DMR:PAYUDMT:USERNAME"],
                    PASSWORD = Configuration["DMR:PAYUDMT:PASSWORD"],
                    PAYOUTID = Configuration["DMR:PAYUDMT:PAYOUTID"],
                    BASEURL = Configuration["DMR:PAYUDMT:BaseURL"],
                    AUTHURL = Configuration["DMR:PAYUDMT:AUTHURL"]
                };
            }
            catch (Exception ex)
            {
        ...
        private void TokenGeneration()
        {
            ...
            {
                if ((EXPIRES_IN - (DateTimeOffset.UtcNow.ToUnixTimeMilliseconds() - TOKEN_CREATED) / 1000) < 5)
                {
                    var req = new PayUTokenRequest
                    {
                        client_id = appSetting.CLIENTID,
        ...
        public DMRTransactionResponse AccountTransfer(MTAPIRequest request)
        {
            ...
            var senderRequest = (SenderRequest)new ProcGetSender(_dal).Call(request.SenderMobile);
            var dataRequest = new PayUDMTRequest
            {
                agentId = request.UserMobile,
                amount = request.Amount.ToString(),
                bankName = beneficiaryModel.BankName ?? string.Empty,
                beneAccNo = request.mBeneDetail.AccountNo,
                beneMobNo = beneficiaryModel.MobileNo,
                beneName = beneficiaryModel.Name,
                customerMobile = request.SenderMobile,
                distTxnRefNo = "TID" + request.TID,
                ifsc = request.mBeneDetail.IFSC,
                stateCode = request.StateID.ToString().PadLeft(2, '0'),
                custFirstName = (senderRequest.Name ?? string.Empty).Contains(" ") ? senderRequest.Name.Split(' ')[0] : senderRequest.Name ?? string.Empty,
                custLastName = (senderRequest.Name ?? string.Empty).Contains(" ") ? senderRequest.Name.Split(' ')[1] : senderRequest.Name ?? string.Empty,
            ...
            //https://test.payumoney.com/dmt/payment/sync/v2/imps
            string URL = appSetting.BASEURL + "dmt/payment/sync/v2/imps";

            var headers = new Dictionary<string, string>
            {
                { "authorization", "Bearer "+ TOKEN },
                { "payuDistributorId", appSetting.PAYOUTID }
            };
            ...
            try
            {
                ...
                if (apiResp != null)
                {
                    var payUResp = JsonConvert.DeserializeObject<PayUDMTResponse>(apiResp);
                    if (!string.IsNullOrEmpty(payUResp.code))
                    {
                        if (payUResp.code.Equals("000"))
                        {
                            if ((payUResp.status ?? 1) == 0)
                            {
                                if (payUResp.result.txnStatus.Equals(RechargeRespType._FAILED))
                                {
                                    res.LiveID = payUResp.result.errorMsg;
                                    res.Statuscode = RechargeRespType.FAILED;
                                    res.Msg = nameof(DMTErrorCodes.Transaction_Failed).Replace("_", " ");
                                    res.ErrorCode = DMTErrorCodes.Transaction_Failed;
                                }
                                else if (payUResp.result.txnStatus.Equals(RechargeRespType._SUCCESS))
                                {
                                    res.Statuscode = RechargeRespType.SUCCESS;
                                    res.Msg = nameof(ErrorCodes.Transaction_Successful).Replace("_", " ");
                                    res.ErrorCode = ErrorCodes.Transaction_Successful;
                                    res.LiveID = payUResp.result.bankRefNo ?? string.Empty;
                                    res.VendorID = payUResp.result.payuTxnRefNo ?? string.Empty;
                                }
                                else if (payUResp.result.txnStatus.Equals(RechargeRespType._PENDING))
                                {
                        ...
                        {
                            res.LiveID = (payUResp.description??string.Empty).Contains("insuf")? nameof(ErrorCodes.ServiceDown): payUResp.description;
                            res.Statuscode = RechargeRespType.FAILED;
