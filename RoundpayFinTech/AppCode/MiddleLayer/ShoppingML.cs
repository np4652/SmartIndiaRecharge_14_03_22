...
{
    public class ShoppingML : BaseML, IShoppingML
    {
        private object validate;
        private readonly IRequestInfo _rinfo;

        public ShoppingML(IHttpContextAccessor accessor, IHostingEnvironment env, bool IsInSession = true) : base(accessor, env, IsInSession)
        {
            _rinfo = new RequestInfo(accessor, env);
        }

        private void WriteLocalSession(object value, bool isPersistent)
        {
            CookieHelper cookie = new CookieHelper(_accessor);
            cookie.Set(SessionKeys.ECommUserDetail, JsonConvert.SerializeObject(value), isPersistent ? DateTime.Now.AddDays(30) : DateTime.Now.AddHours(1));
        ...
        private ECommUserDetail ReadLocalSession(string key= SessionKeys.ECommUserDetail)
        {
            CookieHelper cookie = new CookieHelper(_accessor);
        ...
        public ResponseStatus GetECommVendorLocation(int id, int userid = 0)
        {
            ResponseStatus res = new ResponseStatus
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            IProcedure proc = new ProcGetEComVendorLocation(_dal);
            var response = (ResponseStatus)proc.Call(req);
            return response;
        }
        public ResponseStatus UpdateECommVendorLocation(int id, string latitude, string longitude, int userid = 0)
        {
            ResponseStatus res = new ResponseStatus
            {
                Statuscode = ErrorCodes.Minus1,
            ...
            IProcedure proc = new ProcUpdateEComVendorLocation(_dal);
            var response = (ResponseStatus)proc.Call(req);
            return response;
        }
        #endregion
        ...
        public ShoppingShipping GetShippingAddressByID(int ID)
        {
            if (_lr.LoginTypeID == LoginType.ApplicationUser)
            {
                var commonReq = new CommonReq
                ...
                IProcedure _proc = new ProcGetShippingDetailByID(_dal);
                return (ShoppingShipping)_proc.Call(commonReq);
            }
            return new ShoppingShipping();
        }
        public ResponseStatus DeleteShippingAddress(int ID)
        {
            ...
            if (_lr.LoginTypeID == LoginType.ApplicationUser)
            {
                ...
                IProcedure _proc = new ProcDeleteShippingAddress(_dal);
        ...
        public IEnumerable<FilterWithOptions> GetRequeiredFilter(int cid, int sid, int sid2, string filters)
        {
            var req = new CommonReq
            {
                LoginID = _lr != null ? _lr.UserID : 0,
                CommonInt = sid2 != null ? sid2 : 0,
                CommonInt2 = sid != null ? sid : 0,
                CommonInt3 = cid != null ? cid : 0,
                CommonStr = filters != null ? filters : "0"
            };
        ...
        public IEnumerable<ProductDetail> GetFilteredProduct(ProductFilter p)
        {
            ...
            p.FilterIds = FIds;
            p.OptionIds = OIds;
            p.BrandIDs = p.BrandIDs == null ? "" : p.BrandIDs;
            p.PriceRange = p.PriceRange == null ? "" : p.PriceRange;
        ...
        {
            var rinfo = _rinfo.GetBrowserFullInfo();
            var req = new CommonReq
            {
        ...
        public async Task<IEnumerable<RecentViewModel>> RecentViewDetails(int UserID = 0,string WebSiteID="1")
        {
            ...
            try
            {
                if (_lr != null || UserID > 0)
                {
                    ...
                    res = (List<RecentViewModel>)proc.Call(new CommonReq { CommonStr = WebSiteID, CommonInt2 = UserID }).Result;
                }
                else
                {
                    var eCommUser = ReadLocalSession(SessionKeys.ECommRecentView);
                    if (eCommUser == null || eCommUser.RecentView == null)
                    {
                        return res;
                    }
                    else
                    {
                        ...
                        IProcedureAsync proc = new ProcRecentViewExternal(_dal);
                        res = (List<RecentViewModel>)await proc.Call(req).ConfigureAwait(false);
                    }
                }
            }
            catch (Exception ex)
        ...
        public ResponseStatus AddToWishList(int ProductDetailID,int LoginId=0)
        {
            var req = new CommonReq
            {
                LoginID = LoginId,//_lr != null ? _lr.UserID : 0,
                CommonInt = ProductDetailID
            };
        ...
        public ResponseStatus RemoveFromWishList(int ProductDetailID,int LoginId, int id = 0, bool RemoveAll = false)
        {
            var req = new CommonReq
            {
                LoginID = LoginId,//_lr != null ? _lr.UserID : 0,
        ...
        public IResponseStatus ChangePartialOrderStatus(ChangeOrderStatus req)
        {
            ...
            var response = (OrderStatusResp)proc.Call(req);
            if (req.StatusID == (int)OrderStatus.DeliveryAssigned && response.Statuscode == ErrorCodes.One)
            {
                SendSMSML sendSmsMl = new SendSMSML(_accessor, _env);
                foreach (var item in response.deliveryAlerts)
                {
                    string Notification = "New Delivery for " + item.VendorLat + "," + item.VendorLong;
                    var alertRes = sendSmsMl.SendDeliveryNotification(item.Token, Notification, null, "New Delivery", null, item.OrderDetailId.ToString(), "new_delivery");
                }
            }
            var res = new ResponseStatus
            {
