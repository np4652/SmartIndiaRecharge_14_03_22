...
namespace Fintech.AppCode.DB
{
    public class DAL : IDAL
    {
        protected readonly string _ConStr;
        public DAL(string ConStr) => _ConStr = ConStr;
        public string GetDBNameFromIntialCatelog()
        {
            string s = string.Empty;
            if ((_ConStr ?? string.Empty).Contains("Initial Catalog=")) {
                if (_ConStr.Contains(";User ID")) {
                    var ss = _ConStr.Split(";User ID")[0];
                    s=ss.Split("Initial Catalog=")[1];
                }
            }
            return s;
        }
