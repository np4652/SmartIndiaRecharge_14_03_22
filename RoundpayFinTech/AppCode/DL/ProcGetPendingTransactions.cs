...
{
    public class ProcGetPendingTransactions : IProcedureAsync
    {
        private readonly IDAL _dal;
        public ProcGetPendingTransactions(IDAL dal) => _dal = dal;
        public async Task<object> Call(object obj)
        {
            ...
            SqlParameter[] param = {
                ...
                new SqlParameter("@ReportType", _req.CommonInt3),
                new SqlParameter("@SenderMobile", _req.CommonStr??"")
            };
                ...
                if (Convert.ToInt32(dt.Rows[0][0]) == ErrorCodes.One)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        var transaction = new PendingTransaction
                        {
                            ...
                            APIType = Convert.ToInt32(row["_ApiType"] is DBNull ? 0 : row["_ApiType"]),
                            ...
                            ExtraParam = row["_ExtraParam"] is DBNull ? "" :row["_ExtraParam"].ToString(),
                        };
        ...
        public string GetName() => "proc_GetPendingTransactions";
    }
}
