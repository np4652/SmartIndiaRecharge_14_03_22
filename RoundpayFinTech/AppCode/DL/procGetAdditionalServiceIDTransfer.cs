...
{
    public class procGetAdditionalServiceIDTransfer : IProcedure
    {
        private readonly IDAL _dal;
        public procGetAdditionalServiceIDTransfer(IDAL dal) => _dal = dal;
                    ...
                    if (Convert.ToInt32(dt.Rows[0][0]) == ErrorCodes.One)
                    {
                        foreach (DataRow row in dt.Rows)
                        {
                            data.Add(new AddonServ
                            {
                                ...
                                IDLimit = row["IDLimit"] is DBNull ? 0 : Convert.ToInt32(row["IDLimit"]),
                            });
                        }
                    }
        ...
        public string GetName() => "proc_GetAdditionalServiceIDTransfer";
    }
}
