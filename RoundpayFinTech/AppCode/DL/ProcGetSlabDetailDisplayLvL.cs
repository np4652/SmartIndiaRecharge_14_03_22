                ...
                if (dt.Rows.Count > 0 && !dt.Columns.Contains("Msg"))
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        slabCommissions.Add(new SlabCommission
                        {
                            ...
                            SCode = row["_SCode"] is DBNull ? "" : Convert.ToString(row["_SCode"]),
                            AllowChannel = row["_AllowedChannel"] is DBNull ? "" : row["_AllowedChannel"].ToString(),
                            IsSpecialOp = row["IsSpecialOp"] is DBNull ? false : Convert.ToBoolean(row["IsSpecialOp"])
                        });
                    }
