...
{
    public class Proc_GetSlabCommGI : IProcedure
    {
        private readonly IDAL _dal;
        public Proc_GetSlabCommGI(IDAL dal) => _dal = dal;
                ...
                if (dt.Rows.Count > 0 && !dt.Columns.Contains("Msg"))
                {
                    ...
                    foreach (DataRow row in dt.Rows)
                    {
                        var slabCommission = new SlabCommission
                        {
                            ...
                            ComOnAmtType = row["ComOnAmtType"] is DBNull ? 0 : Convert.ToInt32(row["ComOnAmtType"]),
                            TPComm = row["TPCommission"] is DBNull ? 0 : Convert.ToDecimal(row["TPCommission"]),
                            TPCommType = row["TPCommType"] is DBNull ? 0 : Convert.ToInt32(row["TPCommType"]),
                            TPAmtType = row["TPAmtType"] is DBNull ? 0 : Convert.ToInt32(row["TPAmtType"]),
                            TPComOnAmtType = row["TPComOnAmtType"] is DBNull ? 0 : Convert.ToInt32(row["TPComOnAmtType"]),
        ...
        public string GetName() => "proc_GetGICommissionDetial";
    }
