...
{
    public class ProcGetSlabDetailForCircleAPI : IProcedure
    {
        private readonly IDAL _dal;
        public ProcGetSlabDetailForCircleAPI(IDAL dal) => _dal = dal;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        {
                            APIID=row["_APIID"] is DBNull? req.CommonInt:Convert.ToInt32(row["_APIID"]),
        ...
        public string GetName() => "proc_GetSlabDetailForCircleAPI";
    }
}
