...
{
    public class ProcGetIncentiveByOperator : IProcedure
    {
        private readonly IDAL _dal;
        public ProcGetIncentiveByOperator(IDAL dal) => _dal = dal;
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        var daybook = new Daybook
                        {
                            ...
                            Denomination = Convert.ToInt32(row["Denomination"] is DBNull ? 0 : row["Denomination"]),
        ...
        public string GetName() => "proc_GetIncentiveByOperator";
    }
}
