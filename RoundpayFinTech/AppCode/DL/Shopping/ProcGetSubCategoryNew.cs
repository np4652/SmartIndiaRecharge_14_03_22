...
{
    public class ProcGetSubCategoryNew : IProcedure
    {
        private readonly IDAL _dal;

        public ProcGetSubCategoryNew(IDAL dal) => _dal = dal;
    ...
    public class ProcGetSubCategoryByIdNew : IProcedure
    {
        private readonly IDAL _dal;

        public ProcGetSubCategoryByIdNew(IDAL dal) => _dal = dal;

        public object Call(object obj)
        {
            var res = new List<SubCategory>();
            CommonReq req = (CommonReq)obj;
            SqlParameter[] param = {
                new SqlParameter("@LoginID", req.LoginID),
                new SqlParameter("@lt", req.LoginTypeID),
                new SqlParameter("@ParetId", req.CommonInt),
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow dr in dt.Rows)
                    {
                        var data = new SubCategory
                        {
                            ...
                            ParentId = Convert.ToInt32(dr["_ParentId"], CultureInfo.InvariantCulture),
                            CategoryID = Convert.ToInt32(dr["_MainCategoryId"], CultureInfo.InvariantCulture),
                            //  SubCategoryName = dr["_SubCategoryName1"] is DBNull ? "" : dr["_SubCategoryName1"].ToString(),
                            //  SubCategoryIDLvl2 = Convert.ToInt32(dr["_ID"], CultureInfo.InvariantCulture),
                            //  SubCategoryNameLvl2 = dr["_Name"] is DBNull ? "" : dr["_Name"].ToString(),
                            IsActive = dr["_IsActive"] is DBNull ? false : Convert.ToBoolean(dr["_IsActive"], CultureInfo.InvariantCulture),
                          //  IsNextLevelExists = dr["_IsNextLevel"] is DBNull ? false : Convert.ToBoolean(dr["_IsNextLevel"], CultureInfo.InvariantCulture),
                            Commission = dr["_Commission"] is DBNull ? 0 : Convert.ToDecimal(dr["_Commission"], CultureInfo.InvariantCulture),
                            CommissionType = dr["_CommissionType"] is DBNull ? false : Convert.ToBoolean(dr["_CommissionType"], CultureInfo.InvariantCulture),
        ...
        public object Call() => throw new NotImplementedException();
        public string GetName() => "proc_GetSubCategoryNew";
    }
}
